...
import { LogOut, MessageCircle, Network, Wallet } from "lucide-react";
...
function ModuleTile({
  ...
  return (
    <Link
      href={href}
      className="group relative flex min-h-[118px] flex-col rounded-md border border-[#d5dce6] bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition hover:border-pragma-electric/40 hover:shadow-md"
    >
      <span className="inline-flex items-center gap-2 text-sm font-medium text-[#4a5568]">
        <span className="size-2 rounded-full bg-pragma-electric" />
        {label}
      </span>
      <div className="pointer-events-none absolute bottom-3 right-3 flex size-20 items-center justify-center">
        <div className="absolute inset-0 rounded-full border border-[#c5d5e8]/60" />
        <div className="absolute inset-2 rounded-full border border-[#d5e3f0]/80" />
        <Icon className="relative size-9 text-[#5a6f85]" strokeWidth={1.5} />
      </div>
    </Link>
  );
}
...
export function TiendasOnHub({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[#f3f5f7]">
      <header className="flex h-12 items-center justify-between border-b border-[#d9dee5] bg-white px-4">
        <div className="flex-1" />
        <p className="truncate px-4 text-center text-sm font-medium uppercase tracking-wide text-[#4a5568]">
          {storeName}
        </p>
        <div className="flex flex-1 items-center justify-end gap-3 text-[#5a6f85]">
          <span className="rounded bg-pragma-electric/10 px-2 py-0.5 text-[10px] font-bold uppercase text-pragma-electric">
            PRAGMA
          </span>
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <aside className="relative z-10 hidden w-[280px] shrink-0 flex-col justify-center px-8 lg:flex">
          ...
          {userLabel ? (
            <p className="mt-4 text-xs uppercase tracking-wider text-[#718096]">{userLabel}</p>
          ) : null}
        </aside>
        ...
        <main className="relative z-10 flex min-w-0 flex-1 flex-col p-4 lg:p-6">
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-full border border-[#c5ced8] bg-white px-4 py-2 text-sm text-[#4a5568]"
            >
